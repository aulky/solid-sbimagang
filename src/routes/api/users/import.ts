...
export async function POST({ request }: APIEvent) {
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: "Akses ditolak. Anda bukan admin." }), {
      status: 403,
  ...
  try {
    ...
    if (!file || file.size === 0) {
      return new Response(JSON.stringify({ error: "File tidak valid atau kosong." }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    if (file.size > 2 * 1024 * 1024) {
      return new Response(JSON.stringify({ error: "Ukuran file maksimal 2MB." }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    if (!wb || !wb.SheetNames || wb.SheetNames.length === 0) {
      return new Response(JSON.stringify({ error: "Format Excel tidak valid atau kosong." }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    if (!sheet) {
      return new Response(JSON.stringify({ error: "Sheet pertama tidak ditemukan dalam file." }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    if (rows.length === 0) {
      return new Response(JSON.stringify({ error: "Template kosong, tidak ada data." }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (usedUsernames.has(uLower) || batchUsernames.has(uLower)) {
        errors.push({ row: rowNum, username, error: "Username sudah terdaftar." });
        continue;
      }
    ...
    return new Response(JSON.stringify({ error: error.message || "Gagal mengimpor data." }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
  }
