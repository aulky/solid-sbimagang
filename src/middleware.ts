  ...
  onRequest: [
    async (event) => {
      ...
      if (maliciousPatterns.some((pattern) => checkStr.includes(pattern))) {
        ...
        console.warn(
          `[SECURITY ALERT] Blocked malicious request: ${method} ${url.href} from ${ip}`,
        );
      ...
      if (["POST", "PUT", "DELETE", "PATCH"].includes(method)) {
        ...
        if (!targetHost || targetHost !== host) {
          console.warn(
            `[SECURITY ALERT] CSRF blocked: Origin/Referer (${targetHost || "missing"}) does not match Host (${host})`,
          );
